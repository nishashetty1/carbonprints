...
// This code expects the service account JSON to be in an environment variable.
...
app.post("/api/submit", async (req, res) => {
  ...
  try {
    ...
    await responsesCollection.add({ ...formData, timestamp });
    res
      .status(201)
      .json({ message: "Data submitted successfully to Firebase!" });
  } catch (error) {
...
// Route to get aggregated responses
app.get("/api/responses", async (req, res) => {
  try {
    ...
    if (snapshot.empty) {
      return res
        .status(200)
        .json({ departmentData: [], overallAverage: 0, transportData: {} });
    }
    ...
    const calculateFootprint = (formData) => {
      ...
      const cabEmissions =
        (EMISSION_FACTORS.cab[formData.CabUsage] || 0) * 0.15; // Assume cab factor
      const flightEmissions =
        (EMISSION_FACTORS.flights[formData.FlightsPerYear] || 0) / 12; // Monthly
      breakdown.travel =
        primaryTravelEmissions + cabEmissions + flightEmissions;
      ...
      const wasteEmissions =
        EMISSION_FACTORS.waste[formData.WasteSegregation] || 0;
    ...
    const dataWithFootprints = allData.map((d) => ({
      ...d,
      ...calculateFootprint(d),
    }));
    const totalFootprint = dataWithFootprints.reduce(
      (acc, curr) => acc + curr.totalFootprint,
      0
    );
    const overallAverage =
      dataWithFootprints.length > 0
        ? totalFootprint / dataWithFootprints.length
        : 0;

    const deptMap = new Map();
    dataWithFootprints.forEach((item) => {
      const dept = item.Department;
      if (dept) {
        if (!deptMap.has(dept)) deptMap.set(dept, { total: 0, count: 0 });
        const current = deptMap.get(dept);
        current.total += item.totalFootprint;
        current.count += 1;
      }
    });
    ...
    const transportData = dataWithFootprints.reduce((acc, curr) => {
      const transport = curr.PrimaryTransport;
      if (transport) acc[transport] = (acc[transport] || 0) + 1;
      return acc;
    }, {});

    res.status(200).json({ departmentData, overallAverage, transportData });
  } catch (error) {
    console.error("Error reading from Firestore:", error);
    res.status(500).json({ message: "Server error. Failed to process data." });
  }
});
